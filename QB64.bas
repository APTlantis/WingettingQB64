...
DIM SHARED ResultName(1 TO 2000) AS STRING
DIM SHARED ResultId(1 TO 2000) AS STRING

DIM SHARED QueueCount AS LONG
DIM SHARED QueueName(1 TO 2000) AS STRING
DIM SHARED QueueId(1 TO 2000) AS STRING

': External modules: ---------------------------------------------------------------
'$INCLUDE:'InForm\InForm.bi'
'$INCLUDE:'InForm\clojure.uitheme'
'$INCLUDE:'QB64.frm'
...
SUB __UI_OnLoad
    ...
    SetCaption DownloadPackagesBT, "Install Queue"
    SetCaption SaveThisListBT, "Save Queue"
    Text(SearchTB) = ""
    SelectedPackageName = ""
    SelectedPackageId = ""
    LastStatusMessage = ""
...
SUB __UI_Click (id AS LONG)
    ...
    SELECT CASE id
        ...
        LogDebug "Import button clicked."
            ImportQueueList
...
CASE WinGetting
        ...
        CASE AddToListBT
            LogDebug "Add to list (queue) clicked. selectedId=" + SelectedPackageId
            AddToQueue
        ...
        CASE DownloadPackagesBT
            LogDebug "Install Queue button clicked."
            InstallQueue

        CASE SaveThisListBT
            LogDebug "Save Queue button clicked."
            SaveQueue
...
SUB AddToQueue
    IF LEN(SelectedPackageId) = 0 THEN
        SetStatus "Select a package first to add to queue."
        EXIT SUB
    END IF
    IF QueueCount < UBOUND(QueueName) THEN
        QueueCount = QueueCount + 1
        QueueName(QueueCount) = SelectedPackageName
        QueueId(QueueCount) = SelectedPackageId
        AddItem ListBox2, SelectedPackageName + " [" + SelectedPackageId + "]"
        SetStatus "Added " + SelectedPackageName + " to queue."
        Notify "Added " + SelectedPackageName + " to queue."
    ELSE
...
SUB InstallQueue
    ...
    IF QueueCount = 0 THEN
        SetStatus "Queue is empty. Add packages first."
        EXIT SUB
    END IF

    outputFile = ProjectFile$("winget_install_output.txt")
    
    FOR i = 1 TO QueueCount
        DeleteFileIfExists outputFile
        SetStatus "Installing (" + LTRIM$(STR$(i)) + "/" + LTRIM$(STR$(QueueCount)) + "): " + QueueName(i) + "..."
        _DISPLAY
        
        cmdLine = "winget install --id " + CHR$(34) + QueueId(i) + CHR$(34) + " -e --accept-package-agreements --accept-source-agreements > " + CHR$(34) + outputFile + CHR$(34) + " 2>&1"
        LogDebug "Queue Install start cmd=" + cmdLine
        SHELL _HIDE "cmd /c " + cmdLine

        IF InstallLooksSuccessful(outputFile) THEN
            LogDebug "Install success detected for " + QueueName(i)
        ELSE
            LogDebug "Install success not detected for " + QueueName(i)
        END IF
    NEXT
    
    SetStatus "Finished installing queue."
    Notify "Finished installing queue."
...
SUB SaveQueue
    ...
    IF QueueCount = 0 THEN
        SetStatus "Queue is empty. Add packages first to save."
        EXIT SUB
    END IF

    saveFile = ProjectFile$("saved_packages.txt")
    ...
    SetStatus "Saved " + LTRIM$(STR$(QueueCount)) + " packages to saved_packages.txt!"
    Notify "Saved packages to saved_packages.txt!"
END SUB
...
SUB ImportQueueList
    ...
    importFile = ProjectFile$("saved_packages.txt")
    IF _FILEEXISTS(importFile) = 0 THEN
        SetStatus "No saved_packages.txt found to import."
        EXIT SUB
    END IF

    OPEN importFile FOR INPUT AS #3
    DO UNTIL EOF(3)
        LINE INPUT #3, rawLine
        rawLine = _TRIM$(rawLine)
        
        ' Ignore empty lines or comments if any
        IF LEN(rawLine) > 0 THEN
            ' Very flexible parsing: assume the main piece is the first chunk before spaces
            ' Or if it's just ID, just take it. Let's assume the string holds valid IDs since winget handles spaces.
            pkgId = rawLine
            IF QueueCount < UBOUND(QueueName) THEN
                QueueCount = QueueCount + 1
                QueueName(QueueCount) = pkgId
                QueueId(QueueCount) = pkgId
                AddItem ListBox2, pkgId + " [Imported]"
            END IF
        END IF
    LOOP
    CLOSE #3

    SetStatus "Imported list from saved_packages.txt!"
    Notify "Imported packages to Queue."
END SUB
...
FUNCTION ProjectFile$ (fileName AS STRING)
    ProjectFile$ = "D:\QB-Winget\" + fileName
END FUNCTION
